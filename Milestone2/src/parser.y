...
atom_expr: atom optional_trailer {
  ...
  if(temp!=NULL){
    ...
    for(i=0; i<function_argument.size(); i++){
      ...
      if(i<temp->parameter_types.size()&&!type_correct(function_argument[i],temp->parameter_types[i])){
    ...
    if(($2).is_array == 1&&is_func_param==0&&($1).lexeme!="list") {
      string a = newtemp();
...
optional_comma_arguement: COMMA argument{
  if(is_paren==1){
   ...
   function_arguments.push_back(($2).type);
      if(!func.empty()) func.pop();
    func.push(function_arguments);
  }
    $$.type=($2).type;
